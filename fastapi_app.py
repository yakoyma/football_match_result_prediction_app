...
async def get_prediction(HomeTeam: str, AwayTeam: str, HomeTeamScore: int,
    ...
    try:
        ...
        if (HomeTeam_tokens_length > 0 and AwayTeam_tokens_length > 0 and
            ...
            encoder_pickle = load(encoder_path)
            ...
            answer = (f'The result of the football match between {HomeTeam} '
                      f'and {AwayTeam} is: {result}.')
            response = {'Parameters': params, 'Response': answer}
        else:
            response = {
                'Parameters': params,
                'Error message': 'Invalid input data. Please complete all '
                                 'fields correctly.'
            }

    except Exception as error:
        response = {
            'Error message': f'The following unexpected error occurred: '
                             f'{error}'
        }
    return response
